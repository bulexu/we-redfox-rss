<template>
  <a-spin :loading="feedLoading" tip="正在加载..." style="width:100%;height:100%">
    <a-layout class="foreign-list">
      <a-layout-sider :width="300" :style="{ background:'#fff', borderRight:'1px solid #eee', display:'flex', flexDirection:'column' }">
        <a-card :bordered="false" :title="`${label}订阅`" :head-style="{ padding:'12px 16px', borderBottom:'1px solid #eee', background:'#fff', border:0 }">
          <template #extra>
            <a-button type="primary" size="small" @click="router.push(`/${platform}/subscriptions`)"><template #icon><icon-plus /></template>订阅</a-button>
          </template>
      ...
      <a-layout-content class="main-content">
        <a-page-header :title="activeFeed?.name || '全部'" :subtitle="activeFeedId==='' ? `显示所有${label}订阅的${contentName}` : (activeFeed?.target || '')" :show-back="false">
          <template #extra>
            <a-space>
        ...
        <a-card style="border:0">
          ...
          <a-alert v-else-if="activeFeedId===''" type="success" closable>聚合视图：跨所有{{ label }}关键词订阅展示{{ contentName }}，点击左侧具体订阅可执行同步、编辑等操作</a-alert>
          <a-alert v-else closable>暂无订阅简介</a-alert>
          <div class="search-bar">
            <a-input-search v-model="articleSearchText" class="search-input" :placeholder="`搜索${contentName}标题或内容`" allow-clear @search="searchArticles" @keyup.enter="searchArticles" />
          ...
          >
            ...
            <template #author="{ record }"><div>{{ record.author || '—' }}</div><div v-if="record.author_id" class="small-muted">{{ record.author_id }}</div></template>
            <template #metrics="{ record }">
              <a-tooltip :content="`▶ ${record.read_count||0} · ❤ ${record.liked_count||0} · 💬 ${record.comments_count||0} · 🔖 ${record.collected_count||0} · 🔁 ${record.share_count||0}`">
                <a-space size="mini"><span>▶ {{ record.read_count || 0 }}</span><span>❤ {{ record.liked_count || 0 }}</span><span>💬 {{ record.comments_count || 0 }}</span><span>🔁 {{ record.share_count || 0 }}</span></a-space>
              </a-tooltip>
            </template>
            ...
            <template #content="{ record }"><div class="content-preview">{{ truncate(record.content || record.description, 120) }}</div></template>
          </a-table>
        </a-card>
      </a-layout-content>
    </a-layout>
...
import {
  deleteForeignFeed, listForeignArticles, listForeignFeedArticles, listForeignFeeds,
  triggerForeignSync, updateForeignFeed, type ForeignArticle, type ForeignFeed, type ForeignPlatform,
} from '@/api/foreign'
import { deleteArticle } from '@/api/article'
import { Avatar } from '@/utils/constants'
import { formatTimestamp } from '@/utils/date'

const props = defineProps<{ platform:ForeignPlatform; label:string; contentName:string }>()
const router=useRouter(), route=useRoute()
const feedList=ref<ForeignFeed[]>([]), feedLoading=ref(false), feedSearchText=ref('')
...
const activeFeed=computed(()=>feedList.value.find(item=>item.id===activeFeedId.value))
const truncate=(text:string,length:number)=>!text?'':text.length<=length?text:`${text.slice(0,length-1)}…`
const allFeed=():ForeignFeed=>({ id:'',name:'全部',cover:'/static/logo.svg',intro:`显示所有${props.label}订阅的${props.contentName}`,status:1,kind:'keyword',target:'',max_fetch_count:0,refresh_interval_hours:0,last_publish_time:0,last_cursor:'',sync_time:0,error_count:0,last_error:'',last_error_at:0,created_at:'',updated_at:'' })

const loadFeeds=async()=>{
  feedLoading.value=true
  try {
    const r:any=await listForeignFeeds(props.platform,{ status:statusFilter.value,kw:feedSearchText.value.trim()||undefined,page:feedPagination.current-1,pageSize:feedPagination.pageSize })
    let list:ForeignFeed[]=r.list||[]
    if(kindFilter.value===''&&statusFilter.value===undefined&&!feedSearchText.value.trim()) list=[allFeed(),...list]
...
const articles=ref<ForeignArticle[]>([]), articleLoading=ref(false), articleSearchText=ref('')
...
const columns=computed(()=>{
  const c:any[]=[{title:'封面',slotName:'cover',width:95},{title:'标题/正文',slotName:'title',ellipsis:true,width:340},{title:'作者',slotName:'author',width:160},{title:'互动',slotName:'metrics',width:330},{title:'发布时间',slotName:'publish_time',width:160}]
...
const loadArticles=async()=>{
  ...
  try{
    ...
    const r:any=activeFeedId.value?await listForeignFeedArticles(props.platform,activeFeedId.value,p):await listForeignArticles(props.platform,p)
    articles.value=r.list||[];articlePagination.total=r.total||0
  }catch(e:any){Message.error(e?.message||String(e)||`获取${props.contentName}失败`)}finally{articleLoading.value=false}
}
...
const openWork=(record:ForeignArticle)=>record.url?window.open(record.url,'_blank','noopener,noreferrer'):window.open(`/views/article/${record.id}`,'_blank')
const openRss=(format:string)=>activeFeedId.value&&window.open(`/feed/${activeFeedId.value}.${format}`,'_blank')

const batchDelete=()=>Modal.confirm({title:'确认批量删除',content:`确定删除选中的 ${selectedRowKeys.value.length} 条内容吗？`,onOk:async()=>{await Promise.all(selectedRowKeys.value.map(id=>deleteArticle(id)));selectedRowKeys.value=[];Message.success('已删除');loadArticles()}})
const editVisible=ref(false)
const editForm=reactive({id:'',target:'',name:'',cover:'',intro:'',max_fetch_count:20,refresh_interval_hours:6,status:1})
const editFeed=(f:ForeignFeed)=>{Object.assign(editForm,{id:f.id,target:f.target,name:f.name,cover:f.cover,intro:f.intro,max_fetch_count:f.max_fetch_count,refresh_interval_hours:f.refresh_interval_hours,status:f.status});editVisible.value=true}
const saveEdit=async()=>{try{await updateForeignFeed(props.platform,editForm.id,{name:editForm.name,avatar:editForm.cover,intro:editForm.intro,max_fetch_count:editForm.max_fetch_count,refresh_interval_hours:editForm.refresh_interval_hours,status:editForm.status});editVisible.value=false;Message.success('已保存');loadFeeds()}catch(e:any){Message.error(e?.message||String(e)||'保存失败')}}
const toggleStatus=async(f:ForeignFeed)=>{try{await updateForeignFeed(props.platform,f.id,{status:f.status?0:1});Message.success(f.status?'已停用':'已启用');loadFeeds()}catch(e:any){Message.error(e?.message||String(e)||'操作失败')}}
const syncFeed=async(id:string)=>{try{await triggerForeignSync(props.platform,id);Message.success('已提交同步任务')}catch(e:any){Message.error(e?.message||String(e)||'同步失败')}}
const removeFeed=(f:ForeignFeed)=>Modal.confirm({title:'删除订阅',content:`确定删除 ${f.target} 吗？将同时清理所有关联内容。`,okButtonProps:{status:'danger'},onOk:async()=>{await deleteForeignFeed(props.platform,f.id);if(activeFeedId.value===f.id)activeFeedId.value='';await loadFeeds();await loadArticles();Message.success('已删除')}})
...
watch(()=>props.platform,async()=>{activeFeedId.value='';feedPagination.current=1;articlePagination.current=1;feedSearchText.value='';articleSearchText.value='';await loadFeeds();await loadArticles()})
