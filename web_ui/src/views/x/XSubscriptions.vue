<template>
  <div class="page">
    <a-page-header title="添加X关键词订阅" subtitle="使用 Redfox 持续采集匹配关键词的最新推文" :show-back="true" @back="router.push('/x/feeds')" />
    <a-card>
      <a-alert type="info" class="notice">X目前只支持关键词订阅，固定使用 Latest（最新）模式；该接口不提供 exactMatch 参数。</a-alert>
      <a-form :model="form" layout="vertical" @submit="createKeyword">
        <a-form-item label="搜索关键词" required><a-input v-model="form.target" placeholder="例如：AI / cycling" allow-clear /></a-form-item>
...
const createKeyword = async () => {
  ...
  creating.value = true
  try {
    await createXFeed({ kind:'keyword', target:form.target.trim(), name:form.name.trim() || undefined, max_fetch_count:form.max_fetch_count, refresh_interval_hours:form.refresh_interval_hours })
    Message.success('已订阅，X首次采集已提交')
    router.push('/x/feeds')
