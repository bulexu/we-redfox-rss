...
export const getSubscriptions = (params?: { page?: number; pageSize?: number; kw?: string; status?: number }) => {
  ...
  return http.get<SubscriptionListResult>('/mps', { params: apiParams })
}

export const getSubscriptionDetail = (mp_id: string) => {
  return http.get<{code: number, data: Subscription}>(`/mps/${mp_id}`)
}

// 添加订阅公众号信息
export const addSubscription = (data: AddSubscriptionParams) => {
  return http.post<{code: number, message: string}>('/mps', data)
}
export const getSubscriptionInfo = (url: string) => {
  return http.post<{code: number, message: string}>(`/mps/by_article?url=${url}`)
}

export const addFeaturedArticle = (data: { url: string }) => {
  return http.post<{code: number, data: FeaturedArticleTask}>('/mps/featured/article', data)
}

export const getFeaturedArticleTaskStatus = (taskId: string) => {
  return http.get<{code: number, data: FeaturedArticleTask}>(`/mps/featured/article/tasks/${taskId}`)
}

export const deleteMpApi = (mp_id: string) => {
  return http.delete<{code: number, message: string}>(`/mps/${mp_id}`)
}
...
export const UpdateMps = (mp_id: string,params: { start_page?: number; end_page?: number }) => {
  ...
  return http.get<{code: number, message: string}>(`/mps/update/${mp_id||'all'}?start_page=${apiParams.start_page}&end_page=${apiParams.end_page}`)
}

// 更新订阅公众号信息
export const updateSubscription = (mp_id: string, data: Partial<Subscription>) => {
  return http.put<{code: number, message: string}>(`/mps/${mp_id}`, data)
}

// 切换公众号状态（启用/禁用）
export const toggleMpStatus = (mp_id: string, status: number) => {
  return http.put<{code: number, message: string}>(`/mps/${mp_id}`, { status })
}
...
export const searchBiz = (kw: string, params: { page?: number; pageSize?: number }) => {
  ...
  return http.get<SubscriptionListResult>(`/mps/search/${kw}`,{ params: apiParams })
}
