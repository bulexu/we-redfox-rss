...
export const getFilterRules = (params?: { feed_id?: string; limit?: number; offset?: number }) => {
  return http.get<FilterRuleListResult>('/filter-rules', { params })
}

// 获取单个过滤规则详情
export const getFilterRule = (ruleId: number) => {
  return http.get<{ code: number; data: FilterRule }>(`/filter-rules/${ruleId}`)
}

// 创建过滤规则
export const createFilterRule = (data: FilterRuleCreateParams) => {
  return http.post<{ code: number; data: { id: number; message: string } }>('/filter-rules', data)
}

// 更新过滤规则
export const updateFilterRule = (ruleId: number, data: FilterRuleUpdateParams) => {
  return http.put<{ code: number; data: { id: number; message: string } }>(`/filter-rules/${ruleId}`, data)
}

// 删除过滤规则
export const deleteFilterRule = (ruleId: number) => {
  return http.delete<{ code: number; data: { id: number; message: string } }>(`/filter-rules/${ruleId}`)
}

// 获取公众号的启用规则
export const getActiveRulesForMp = (mpId: string) => {
  return http.get<{ code: number; data: FilterRule[] }>(`/filter-rules/mp/${mpId}/active`)
}
