...
export const createAccessKey = (data: AccessKeyCreateRequest) => {
  return http.post<AccessKeyResponse>('/auth/ak/create', data)
}

// 获取用户的 Access Keys 列表
export const listAccessKeys = () => {
  return http.get<AccessKeyResponse[]>('/auth/ak/list')
}

// 更新 Access Key
export const updateAccessKey = (akId: string, data: AccessKeyUpdateRequest) => {
  return http.put<AccessKeyResponse>(`/auth/ak/${akId}`, data)
}

// 停用 Access Key
export const deactivateAccessKey = (akId: string) => {
  return http.post(`/auth/ak/${akId}/deactivate`)
}

// 删除 Access Key
export const deleteAccessKey = (akId: string) => {
  return http.delete(`/auth/ak/${akId}`)
}
